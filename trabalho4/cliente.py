...
def m2Decrypt(data, cipherClient, N1):
    ...
    print("Sub M2 descriptografada:")
    print(submensagem1)
    print("Ticket recebido:" + mensagens[1])
...
def m3Encrypt(ticket, sessionKey):
    ...
    encrpty = ID_C + "\n" + ID_S + "\n" + T_R + "\n" + N2
    print("Mensagem 3:" + ID_C + "-" + ID_S + "-" + T_R + "-" + N2)
...
def m5Encrypt(ticket, sessionKey):
    cipher = AES.new(sessionKey, AES.MODE_CBC, b64decode(IV))
    encrpty = ID_C + "\n" + T_R + "\n" + "PING" + "\n" + N3
    print("Mensagem 5:" + ID_C + "-" + T_R + "-" + "PING" + "-" + N3)
    ct_bytes = cipher.encrypt(pad(encrpty.encode(), AES.block_size))
...
def m6Decrypt(data, sessionKey, N3):
    ...
    print("Mensagem 6 - descriptografada:")
    print(mensagens)
    if N3 == mensagens[1]:
        print("Número aleatório recebido correto")
    else:
...
print("Mensagem 1:" + ID_C + "-" + ID_S + "-" + T_R + "-" + N1)
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    print("------------------------------------------------")
    input("Pressione enter para iniciar a comunicação com o AS:")
    print("------------------------------------------------")
    s.connect((HOST, PORT_AS))
    s.sendall(mensagem1.encode())
    data = s.recv(1024)
    print("Mensagem 2 criptografada recebida:", repr(data))
    s.close()
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    print("------------------------------------------------")
    input("Pressione enter para iniciar a comunicação com o TGS:")
    print("------------------------------------------------")
    mensagem3 = m3Encrypt(ticket, sessionKeyTgs)
    s.connect((HOST, PORT_TGS))
    s.sendall(mensagem3.encode())
    data = s.recv(1024)
    print("Mensagem 4 criptografada recebida:", repr(data))
    s.close()
...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    print("------------------------------------------------")
    input("Pressione enter para iniciar a comunicação com o Serviço:")
    print("------------------------------------------------")
    mensagem5 = m5Encrypt(ticket, sessionKeyServico)
    s.connect((HOST, PORT_SERVICO))
    s.sendall(mensagem5.encode())
    data = s.recv(1024)
    print("Mensagem 6 criptografada recebida:", repr(data))
    s.close()
